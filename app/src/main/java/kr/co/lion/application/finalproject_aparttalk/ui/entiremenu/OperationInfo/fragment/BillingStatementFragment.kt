...
import kr.co.lion.application.finalproject_aparttalk.ui.entiremenu.OperationInfo.adapter.BillingStatementRecyclerView
...
class BillingStatementFragment : Fragment() {
    ...
    private val viewModel: BillingStatementViewModel by viewModels()
    ...
    private fun gettingOperationInfoList(){
        CoroutineScope(Dispatchers.Main).launch {
            try {
                ...
                viewModel.gettingBillingStatementList(apartmentId)
                //Log.d("BiddingNoticeFragment", "Fetched list: ${viewModel.biddingNoticeList}")
                fragmentBillingStatementBinding.recyclerViewBillingStatement.adapter?.notifyDataSetChanged()
            } catch (e: Exception) {
    ...
    // 부과명세서 입찰공고 탭 리사이클러뷰 설정
    private fun setRecyclerView() {
        fragmentBillingStatementBinding.recyclerViewBillingStatement.apply {
            adapter = BillingStatementRecyclerView(requireContext(), viewModel)
